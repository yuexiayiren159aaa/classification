...
from utils.myMultiEpochs import MultiEpochsDataLoader
from utils.myMultiEpochs import CudaDataLoader
...
if __name__ == "__main__":
    ...
    #   修改成自己对应的种类的txt
    #----------------------------------------------------#
    classes_path    = 'model_data/cls_imagenet_classes.txt' 
    #----------------------------------------------------#
    #   输入的图片大小
    #----------------------------------------------------#
    input_shape     = [224, 224]
    ...
    #   获得图片路径和标签
    #------------------------------------------------------#
    annotation_path = "data_cls_file/imagenet/cls_train.txt"
    #------------------------------------------------------#
    ...
    loss_history = LossHistory("imagenet_logs/")
    #----------------------------------------------------#
    ...
    if True:
        ...
        gen             = MultiEpochsDataLoader(train_dataset, batch_size=Batch_size, num_workers=num_workers, pin_memory=True,drop_last=True, collate_fn=detection_collate)
        gen_val         = MultiEpochsDataLoader(val_dataset, batch_size=Batch_size, num_workers=num_workers, pin_memory=True,drop_last=True, collate_fn=detection_collate)

        if torch.cuda.is_available():
            gen = CudaDataLoader(gen, 'cuda')
            gen_val = CudaDataLoader(gen_val, 'cuda')
